...
def get_ovf_properties():
    properties={}
    ovfenv_cmd = "/usr/bin/vmtoolsd --cmd 'info-get guestinfo.ovfEnv'"    xml_parts=subprocess.Popen(ovfenv_cmd,shell=True,stdout=subprocess.PIPE).stdout.read()
...
def change_root_passwd(root_password):
    #subprocess.call(["echo "+root_password+ "| /usr/bin/passwd --stdin root"],shell=true)    sp=subprocess.Popen(["/usr/sbin/chpasswd"],stderr=subprocess.PIPE,stdin=subprocess.PIPE)
